    ...
    <div className="app-container gradient-mesh min-h-screen">
      <div className="p-6 space-y-6">
        ...
        <div className="text-center mb-6">
          <Crown className="w-16 h-16 mx-auto text-neon-pink mb-2" />
          <p className="text-dark-purple/70">Unlock all features and endless fun!</p>
        </div>
        ...
        <div className="space-y-4">
          {PLANS.map((plan) => (
            <button
              ...
              className={`w-full card-soft p-6 text-left transition-all ${
                selectedPlan === plan.id ? 'ring-4 ring-neon-pink' : ''
              } ${plan.popular ? 'relative' : ''}`}
            >
              {plan.popular && (
                <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 px-4 py-1 rounded-full bg-neon-pink text-white text-sm font-bold">
                  Most Popular
                </div>
              )}
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="text-2xl font-bold text-dark-purple">{plan.name}</h3>
                  <div className="flex items-baseline gap-2 mt-1">
                    <span className="text-3xl font-bold text-neon-pink">${plan.price}</span>
                    <span className="text-dark-purple/60">USD</span>
                  </div>
                </div>
                <div
              ...
              <ul className="space-y-2">
                {plan.features.map((feature, idx) => (
                  <li key={idx} className="flex items-start gap-2 text-sm text-dark-purple/70">
                    <Check className="w-4 h-4 text-neon-pink mt-0.5 flex-shrink-0" />
                    <span>{feature}</span>
                  </li>
                ))}
              </ul>
        ...
        <button
          ...
          disabled={loading}
          className="w-full neon-button text-lg py-4 disabled:opacity-50"
        >
          {loading ? 'Processing...' : 'Subscribe Now'}
        </button>

        <p className="text-xs text-center text-dark-purple/60">
